...
namespace {
const char PRE_EXTENSION_NAME[] = "log_";
}

CGLogger::CGLogger(const char *argv0, const std::string &log_dir /* = "./"*/)
    : m_app_dir_(GetCurrentAppPath(argv0)), m_logger_path_(log_dir) {
  // FLAGS_log_dir = m_logger_path_;			 // Set log dir, you can
  // delete this
  ...
  google::SetLogFilenameExtension(PRE_EXTENSION_NAME);
  google::SetLogSymlink(google::GLOG_INFO, "");
  google::FlushLogFiles(google::GLOG_ERROR);
  google::FlushLogFilesUnsafe(google::GLOG_FATAL);
}

CGLogger::~CGLogger() { google::ShutdownGoogleLogging(); }
...
const std::string &CGLogger::GetAppDir(void) const { return m_app_dir_; }
